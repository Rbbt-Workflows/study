...
CohortTasks = Proc.new do
  ...
  task :sorted_significant_genes => :array do |threshold|
    ...
    CMD.cmd("env LC_ALL=C sort -k #{field_pos + 1} -g -  | cut -f 1 |grep -v '#'", :pipe => true, :in => Open.open(step(:binomial_significance).path, :nocache => true))
  end
  ...
  task :sorted_damage_biased_genes => :array do |threshold|
    ...
    CMD.cmd("env LC_ALL=C sort -k #{field_pos + 1} -g -  | cut -f 1 |grep -v '#'", :pipe => true, :in => Open.open(step(:gene_damage_bias).path, :nocache => true))
  end
